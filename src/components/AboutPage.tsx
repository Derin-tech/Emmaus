...
export default function AboutPage({ onNavigate }: AboutPageProps) {
  return (
    <div className="dash-root min-h-screen bg-[#F6F2EA] dark:bg-[#1A1817] dark:bg-[#1A1817] text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 sm:py-14">
            ...
            <div className="max-w-xl">
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">
                {[
                  ...
                  <span
                    key={i}
                    className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white dark:bg-[#22201F] dark:bg-[#22201F]/10 px-3 py-1.5 text-[11px] font-semibold text-white/90"
                  >
        ...
        <div className="relative z-10 -mt-6 grid grid-cols-2 gap-4 sm:-mt-8 lg:grid-cols-4">
          {FACTS.map((f, i) => (
            <div key={i} className={`${CARD} p-5`}>
              ...
              <p className="dash-serif mt-3 text-3xl font-semibold leading-none text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">{f.value}</p>
        ...
        <section className="mt-12">
          ...
          <h2 className="dash-serif mt-1 text-2xl font-semibold text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">A scholar devoted to teaching</h2>
        ...
        <section className="mt-10 grid gap-6 md:grid-cols-2">
          ...
          <div className={`${CARD} p-6`}>
            ...
            <h3 className="dash-serif mt-1 text-lg font-semibold text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">Fields of focus</h3>
          ...
          <div className={`${CARD} p-6`}>
            ...
            <h3 className="dash-serif mt-1 text-lg font-semibold text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">Career timeline</h3>
            <ul className="mt-6 space-y-6 border-l border-[#EAE1D2] dark:border-[#4A433E] dark:border-[#383330] pl-5">
              {EXPERIENCE.map((exp, i) => (
                <li key={i} className="relative">
                  ...
                  <span className="mt-1 block text-sm font-bold text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">{exp.title}</span>
                  <span className="mt-0.5 block text-xs text-[#8A7E6F] dark:text-[#A89F91] dark:text-[#A89F91]">{exp.place}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex shrink-0 flex-wrap gap-2.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-white/20 px-4 py-2.5 text-xs font-bold tracking-wide text-white transition-colors hover:bg-white dark:bg-[#22201F] dark:bg-[#22201F]/10"
              >
                Contact office
              </button>
            </div>
