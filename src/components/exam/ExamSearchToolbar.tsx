  ...
  return (
    <div className="flex flex-col items-center gap-8 my-16 animate-[fadeInUp_0.8s_ease-out_forwards]">
      ...
      >
        ...
        <div className="relative flex items-center bg-white/70 backdrop-blur-2xl border border-white rounded-full p-2 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
          <div className={`flex items-center justify-center w-12 h-12 rounded-full transition-colors duration-500 ${isFocused ? 'bg-[#4A0E1B] text-white' : 'bg-transparent text-[#8A7E6F] dark:text-[#A89F91]'}`}>
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none px-4 text-sm sm:text-base text-[#22201F] dark:text-[#F6F2EA] placeholder:text-[#A79A88]"
          />
        </div>
      </div>

      {/* Premium Segmented Control */}
      <div className="flex flex-wrap justify-center p-1.5 bg-white/50 backdrop-blur-xl border border-white rounded-full shadow-sm relative">
        {subjects.map((subject) => (
          <button
            ...
            className={`relative z-10 px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors duration-300 ${
              selectedSubject === subject ? 'text-white' : 'text-[#8A7E6F] dark:text-[#A89F91] hover:text-[#22201F] dark:text-[#F6F2EA]'
            }`}
          >
            {subject}
